...
- For Persian/Farsi responses, format replies for RTL readability because the chat box may not apply RTL automatically. HTML wrappers may not render in this chat. Prefer prefixing each Persian paragraph/list item with the Unicode RTL embedding mark `‫` and ending with `‬`; keep punctuation at the Persian side and avoid mixed English unless necessary.
- User prefers short, direct Persian answers for troubleshooting; avoid long explanations that can be confusing. Give the exact next action first, then only one brief reason if needed.
...
- Context root: `/home/mansour/toobi`
- n8n version at generation time: 2.20.7-exp.0
